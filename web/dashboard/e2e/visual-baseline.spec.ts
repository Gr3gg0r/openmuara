...
const BASELINE_DIR = resolve(REPO_ROOT, 'web', 'dashboard', 'e2e', 'baselines');
...
test.describe('visual baseline', () => {
  for (const theme of ['light', 'dark'] as const) {
    test(`dashboard views match baseline (${theme})`, async ({ page }) => {
      ...
      await page.goto(`${baseURL}/_admin`, { waitUntil: 'networkidle' });
      await expect(page).toHaveScreenshot(`ledger-default-${theme}.png`, { fullPage: true, maxDiffPixelRatio: 0.05 });

      await page.goto(`${baseURL}/_admin?view=webhooks`, { waitUntil: 'networkidle' });
      await expect(page).toHaveScreenshot(`webhooks-view-${theme}.png`, { fullPage: true, maxDiffPixelRatio: 0.05 });

      await page.goto(`${baseURL}/_admin?view=settings`, { waitUntil: 'networkidle' });
      await expect(page).toHaveScreenshot(`settings-view-${theme}.png`, { fullPage: true, maxDiffPixelRatio: 0.05 });

      await page.goto(`${baseURL}/_admin?view=settings&provider=fawry`, { waitUntil: 'networkidle' });
      await expect(page).toHaveScreenshot(`provider-detail-fawry-${theme}.png`, { fullPage: true, maxDiffPixelRatio: 0.05 });

      await page.goto(`${baseURL}/_admin?view=settings&provider=stripe`, { waitUntil: 'networkidle' });
      await expect(page).toHaveScreenshot(`provider-detail-stripe-${theme}.png`, { fullPage: true, maxDiffPixelRatio: 0.05 });
    });
  }
});
