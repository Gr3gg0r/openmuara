...
func TestGetBillTransactionsStatusMapping(t *testing.T) {
	cases := []struct {
		name   string
		status engine.TransactionStatus
		want   string
	}{
		{name: "new_is_pending", status: engine.TransactionStatusNew, want: "2"},
		{name: "paid_is_success", status: engine.TransactionStatusPaid, want: "1"},
		{name: "unpaid_is_failed", status: engine.TransactionStatusUnpaid, want: "3"},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(txs) != 1 || txs[0].BillPaymentStatus != tc.want {
				t.Fatalf("txs = %+v, want billpaymentStatus %q", txs, tc.want)
			}
		})
	}
}
