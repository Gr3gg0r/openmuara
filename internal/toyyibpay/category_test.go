...
func TestCreateCategoryOfficialParams(t *testing.T) {
	p := newTestProvider(t)
	form := url.Values{}
	form.Set("userSecretKey", p.secret)
	form.Set("catname", "Donations")
	form.Set("catdescription", "Monthly donations")
	...
	for _, cat := range p.categories.by {
		if cat.CategoryName != "Donations" || cat.CategoryDescription != "Monthly donations" {
