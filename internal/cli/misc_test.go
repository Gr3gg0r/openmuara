...
func TestDoctorCommandExecution(t *testing.T) {
	...
	// Inject a lookPath that finds every tool so the result does not depend
	// on which binaries are installed on the host (CI runners lack
	// golangci-lint and task, which doctor requires).
	lookPath := func(name string) (string, error) { return "/usr/local/bin/" + name, nil }
	result := runDoctor(cmd, false, lookPath)
	printDoctorResult(&buf, result)

	if !result.Healthy {
		t.Fatalf("expected healthy doctor result: %+v", result)
	}
