...
> **Status:** ✅ Completed | **Started:** 2026-07-03 | **Archived**
...
```
docs/initiatives/archive/openmuara-security-hardening/
├── README.md              # This file
...
└── prompts/               # Numbered, self-contained execution prompts
    ...
    ├── 06-security-cli-helpers.md
    ├── 07-security-scanning-and-ci-gates.md
    ├── 08-automated-security-tests.md
    └── 09-docs-and-runbooks.md
```

Planning docs live in `docs/initiatives/archive/openmuara-security-hardening/` in the root repo.
...
| `MP_HTTP_BIND_ALL` (default false) | `server.host` default `127.0.0.1`; opt-in to `0.0.0.0` |
...
- Use existing `server.host` as the bind address (default `127.0.0.1`).
- Add `server.tls_cert`, `server.tls_key` config.
- Add `hardened: true` preset that enables auth + rate limiting + strict security headers.
...
### Phase 5 — Security tooling, CI gates, and developer helpers

- Add `muara security` CLI commands for local development and CI:
  - `muara security hash-password` — generate bcrypt hash for config.
  - `muara security gen-cert` — generate self-signed TLS cert/key for local testing.
  - `muara security audit` — print a security posture report (bind address, auth state, TLS, rate limits, headers).
- Integrate static security scanners into CI and pre-commit:
  - `gosec ./...` — Go security checker.
  - `govulncheck ./...` — already in CI; keep it.
  - Secret scanning (e.g., `gitleaks`) to prevent committed credentials.
- Add automated security tests: brute-force auth, CSRF bypass, provider endpoint auth bypass, rate-limit enforcement.

---

## Must-Have vs Good-to-Have

### Must-Have (v1 hardening MVP)

1. Default `127.0.0.1` bind with explicit opt-in to `0.0.0.0`.
2. Configurable admin auth (basic auth + bearer token) for `/_admin/*` and admin JSON APIs.
3. Bcrypt password storage; secrets injectable via env vars.
4. `hardened: true` preset.
5. Per-IP rate limiting on admin endpoints.
6. Security headers (CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy).
7. Security audit logging for auth failures, config changes, replay actions.
8. Security scanning in CI (`gosec`, `govulncheck`).
9. CLI helpers (`hash-password`, `gen-cert`, `audit`).
10. Automated security tests.

### Good-to-Have (stretch / future)

1. TLS client-certificate auth.
2. IP allowlist/denylist for admin.
3. OAuth2 / OIDC integration for enterprise environments.
4. Automated cert renewal (Let's Encrypt).
5. SIEM-friendly security event export.
6. Webhook payload encryption at rest.

---

## Security Tools & CI Gates

| Tool | Purpose | Where |
|---|---|---|
| `gosec` | Go security linter (SQL injection, unsafe code, weak crypto, etc.) | CI, optional local |
| `govulncheck` | Vulnerability scanning for Go dependencies | CI (already present) |
| `gitleaks` / `trufflehog` | Prevent committed secrets | CI, pre-commit |
| `scripts/check-forbidden.sh` | Block `fmt.Println`, `os.Exit` in library code | pre-commit, CI |
| `muara security audit` | Runtime posture check | local dev, CI smoke tests |
...
| Command | Purpose |
|---|---|
| `muara security hash-password` | Generate a bcrypt hash to paste into config |
| `muara security gen-cert --host localhost` | Generate self-signed TLS cert/key pair for local HTTPS testing |
| `muara security audit` | Print security configuration and warn on insecure settings |

---

## Memory-Efficient & Fast Design

Security features must not compromise OpenMuara's local-first, low-overhead character:

- **In-memory rate limiter** with bounded map size and per-entry TTL (token bucket or sliding window). No external Redis.
- **No extra goroutines** for security unless necessary; reuse the existing HTTP server lifecycle.
- **Lazy auth initialization** — bcrypt, TLS, and rate-limiter state are only allocated when auth/hardening is enabled.
- **Constant-time password comparison** via bcrypt to prevent timing attacks.
- **CSP and security headers** applied via a single middleware function with negligible overhead.
- **Secure CSRF cookie flags** (`HttpOnly`, `Secure` when TLS is on, `SameSite=Strict`) for admin UI.
- **Request body size limits and timeouts** on admin endpoints to prevent memory exhaustion.
- **Audit logging** reuses the existing `internal/audit` SQLite store; no separate connection.

---

## Testing Strategy

- **Unit tests** for config validation, middleware, and auth helpers.
- **Integration tests** that start the server in hardened mode and verify:
  - `/_admin` requires auth.
  - Provider endpoints remain unauthenticated.
  - Rate limiting blocks brute-force attempts.
  - CSRF protection still works.
  - Security headers are present.
- **CI security job** runs `gosec` and `govulncheck`.
- **Smoke tests** run `muara security audit` and verify it reports insecure defaults.
...
- Do not add external dependencies such as Redis, LDAP, or SSO providers.
- Do not increase baseline memory use when hardening is disabled.
