...
6. Import the new provider package in `internal/cli/start.go` so the factory is
   ...
   ```go
   _ "github.com/openmuara/openmuara/internal/<name>"
   ```
...
The repository may still be cloned into a local directory named `toyol` for
historical reasons. Module paths and user-facing names use
`github.com/openmuara/openmuara` and `OpenMuara` respectively. All committed
documentation and code should use the OpenMuara branding.
