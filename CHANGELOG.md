...
- Visual baseline capture script (`npm run test:visual-baseline`) and captured
  dashboard findings under `docs/initiatives/openmuara-bug-hunt/findings/visual-baseline/`.
...
- Webhook dispatcher chaos tests for non-2xx responses, retry exhaustion, and
  timeouts.
- Root `KNOWN_ISSUES.md` synced from deferred bugs in the bug-hunt register.
- `scripts/release-notes.sh` to scrape fixed bug IDs from `TRACKING.md`.
- PR coverage-regression comment workflow.
- Error-code taxonomy in `internal/errcode` adopted across providers and
  webhook dispatch.
- Recurring bug-hunt process documentation in `docs/bug-hunt-process.md`.
...
- Monthly idempotent bug-hunt prep issue workflow (`bug-hunt-prep.yml`).
- `KNOWN_ISSUES.md` sync check (`scripts/check-known-issues.sh`) wired into CI.
