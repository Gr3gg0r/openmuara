...
run_tests() {
    # Exclude vendored JS dependency Go packages and the examples/ tree from
    # coverage calculations. Example apps are demo code; coverage-exemptions.yml
    # already excludes them from per-package floors, so the total gate must not
    # count their uncovered statements either.
    local packages=()
    while IFS= read -r pkg; do
        packages+=("${pkg}")
    done < <(go list ./... | grep -v -E 'web/dashboard/node_modules|website/node_modules|/examples/')
