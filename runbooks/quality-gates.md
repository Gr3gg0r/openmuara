...
- `known-issues` — `scripts/check-known-issues.sh` sync check (warning until stable).
...
- `.github/workflows/bug-hunt-prep.yml` — opens a monthly bug-hunt prep issue idempotently.
...
### Planning-doc consistency check

```bash
./scripts/audit-trackers.sh
```

Verifies that tracker entry points exist, referenced commits are valid, and `muara.yml.example` matches the bundled defaults.
...
Compares dashboard screenshots against baselines in `docs/initiatives/openmuara-bug-hunt/findings/visual-baseline/`. Baselines are captured for both **light** and **dark** themes (`-light.png` and `-dark.png` suffixes). Dynamic elements (e.g., live refresh timestamps) are hidden with the shared `[data-visual-mask]` attribute instead of per-test CSS.
...
Compares deferred bug IDs in `docs/initiatives/openmuara-bug-hunt/KNOWN_ISSUES.md` with root `KNOWN_ISSUES.md`. Runs as a CI warning until stable.
